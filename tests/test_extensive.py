...
class TestExtensive(unittest.TestCase):
    ...
    def test_model_stop_words_only(self):
        """A string with only stop words should produce a neutral coordinate."""
        text = "what is the"
    ...
    def test_db_different_contexts(self):
        """Storing the same text in different contexts should yield different coordinates."""
        text = "power"
        ...
        # "power" is a keyword in both profiles, but its semantic meaning differs.
        # We expect the final coordinates to be different.
        self.assertNotEqual(concept1['power'], concept2['power'])
